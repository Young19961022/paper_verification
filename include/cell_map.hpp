...
public:
    ...
    float                                                  m_ratio_nonzero_line, m_ratio_nonzero_plane;
    float                                                  m_roi_range;
    pcl::PointCloud<pcl::PointXYZ>                         m_accumulated_point_cloud; // The pointcloud sampled from current frame to last frame
    std::set< Mapping_cell_ptr >                           m_set_cell;

    pcl::octree::OctreePointCloudSearch< pcl::PointXYZ >   m_octree = pcl::octree::OctreePointCloudSearch< pcl::PointXYZ >(0.0001);
    unsigned int                                           m_last_update_feature_cells_number = 0;
    ...
    FUNC_T void add_cells(const std::set< Mapping_cell_ptr > &cells_vec)
    {
        ...
        for (typename std::set< Mapping_cell_ptr >::iterator it = cells_vec.begin(); it != cells_vec.end(); it++) {
            // m_set_cell 每个关键帧都有自己的cell set
            m_set_cell.insert(*(it));
            // m_map_pt_cell 每个关键帧都有自己的cell map 哈希表
    ...
    FUNC_T Eigen::Matrix<DATA_TYPE, 3, 3> generate_feature_img(std::vector<Eigen::Matrix<DATA_TYPE, 3, 1>> &feature_vecs_line,
            ...
            Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &feature_img_plane)
    {
        ...
        // cout << "eigen_vector.transpose = \n" << eigen_vector.transpose() << endl;

        for (size_t i = 0; i < feature_vecs_plane.size(); i++) {
            // cout << "feature_vecs_plane = \n" << feature_vecs_plane[ i ] << endl;
        ...
        return eigen_vector.transpose();
    }

    FUNC_T Eigen::Matrix3f extract_feature_mapping_new(std::set< Mapping_cell_ptr > cell_vec,
            Eigen::Matrix< float, Eigen::Dynamic, Eigen::Dynamic > &feature_img_line,
            Eigen::Matrix< float, Eigen::Dynamic, Eigen::Dynamic > &feature_img_plane,
            ...
            int if_recompute = 0)
    {
        ...
        return generate_feature_img(m_feature_vecs_line, m_feature_vecs_plane, feature_img_line, feature_img_plane);
    };

    FUNC_T Eigen::Matrix3f analyze(int if_recompute = 0)
    {
        update_features_of_each_cells();

        return extract_feature_mapping_new(m_set_cell, m_feature_img_line, m_feature_img_plane, m_feature_img_line_roi, m_feature_img_plane_roi, if_recompute);
    }
